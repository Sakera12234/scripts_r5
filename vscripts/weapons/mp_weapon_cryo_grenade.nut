...
const asset BURN_EFFECT_ASSET = $"P_training_water_wall"

const asset VM_EFFECT = $"P_mirage_holo_trail"
const float statuseffect_duration = 3
...
{
	#if CLIENT
	thread FadeModelIntensityOverTime( weapon, 0.50, 0, 255)
	#endif

	weapon.PlayWeaponEffect( VM_EFFECT, $"", "FX_BOTSIDE" )
	weapon.PlayWeaponEffect( VM_EFFECT, $"", "FX_TOPSIDE" )

	weapon.EmitWeaponSound_1p3p( "", "weapon_thermitegrenade_draw_3p" )
}

void function OnWeaponDeactivate_CryoGrenade( entity weapon )
{
	#if CLIENT
	thread FadeModelIntensityOverTime( weapon, 0.25, 255, 0 )
	#endif

	weapon.StopWeaponEffect( VM_EFFECT, $"" )
	weapon.StopWeaponEffect( VM_EFFECT, $"" )
...
void function OnProjectileCollision_weapon_Cryo_grenade( entity projectile, vector pos, vector normal, entity hitEnt, int hitbox, bool isCritical )
{
...
void function DoSegment( entity owner, entity inflictor, SegmentData segment, BurnDamageSettings burnSettings )
{
	...
	entity iceffect = CreatePropDynamic( $"mdl/fx/peacekeeper_legendary_ice_lvl2_add.rmdl", segment.startPos + <0,0,20>, segment.angles  )
	iceffect.SetModelScale(RandomFloatRange(15,17))

	OnThreadEnd(
		function() : ( iceffect, burnSettings )
		{
            thread function() : (iceffect, burnSettings)
			{
				wait burnSettings.burnDuration
				if ( IsValid( iceffect ) )
				{
					//entity effect = PlayFX( $"P_impact_exp_med_water" , iceffect.GetOrigin() +  <0,0,-20>)
					entity effect2 = PlayFX( $"P_boost_jetwash_water" , iceffect.GetOrigin() +  <0,0,-20>)
					EmitSoundAtPosition( TEAM_BOTH, iceffect.GetOrigin(), "Phys_Imp_Default_Water")
					iceffect.Dissolve(ENTITY_DISSOLVE_NONE, < 0, 0, 0 > , 200)
					wait 0.2
					//effect.Destroy()
					effect2.Destroy()
				}
	...
	//entity iceffect = CreateSegmentEffect( BURN_EFFECT_ASSET, owner, segment.startPos, segment.endPos, segment.angles, burnSettings.burnDuration )

	AI_CreateDangerousArea_Static( iceffect, inflictor, burnSettings.burnDamageRadius, TEAM_INVALID, true, true, segment.endPos )

	thread FireSegment_DamageThink( iceffect, owner, inflictor, burnSettings )

	if ( segment.sound != "" )
	{
		EmitSoundOnEntityAfterDelay( iceffect, segment.sound, 0.3 )
		PlayFXOnEntity( $"P_hmn_land_impact_snow", iceffect )
	}
...
void function FireSegment_DamageThink( entity effect, entity owner, entity inflictor, BurnDamageSettings burnSettings )
{
	...
	while ( 1 )
	{
		...
		foreach ( ent in touchingEnts )
		{
			...
			StatusEffect_AddTimed( ent, eStatusEffect.turn_slow, 0.5, statuseffect_duration, 0.5 )
			StatusEffect_AddTimed( ent, eStatusEffect.move_slow, 0.3, statuseffect_duration, 0.5 )
			StatusEffect_AddTimed( ent, eStatusEffect.emp, 0.3, statuseffect_duration, 0.5 )
		}
